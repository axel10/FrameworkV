...
namespace Root
{
    ...
    {
        public Expression<Func<T, bool>> AsExpression<T>(Query.Condition? condition = Query.Condition.AndAlso)
            where T : class
        {
            ...
            Expression Append(Expression exp1, Expression exp2)
            {
                ...
                return (condition ?? Query.Condition.AndAlso) == Query.Condition.OrElse
                ...
                switch (item.Operator)
                {
                    case Query.Operators.Equal:
                    {
                        expression = Append(expression,
                            Expression.Equal(Expression.Property(parameter, item.Name),
                                Expression.Constant(Convert.ChangeType(item.Value, realType), property.PropertyType)));
                        break;
                    }
                    case Query.Operators.GreaterThan:
                    {
                        expression = Append(expression, Expression.GreaterThan(
                            Expression.Property(parameter, item.Name),
                            Expression.Constant(Convert.ChangeType(item.Value, realType), property.PropertyType)));
                        break;
                    }
                    case Query.Operators.GreaterThanOrEqual:
                    {
                        expression = Append(expression, Expression.GreaterThanOrEqual(
                            Expression.Property(parameter, item.Name),
                            Expression.Constant(Convert.ChangeType(item.Value, realType), property.PropertyType)));
                        break;
                    }
                    ...
                    {
                        expression = Append(expression,
                            Expression.LessThan(Expression.Property(parameter, item.Name),
                                Expression.Constant(Convert.ChangeType(item.Value, realType), property.PropertyType)));
                        break;
                    }
                    case Query.Operators.LessThanOrEqual:
                    {
                        expression = Append(expression, Expression.LessThanOrEqual(
                            Expression.Property(parameter, item.Name),
                            Expression.Constant(Convert.ChangeType(item.Value, realType), property.PropertyType)));
                        break;
                    }
                    ...
                    {
                        var nullCheck = Expression.Not(Expression.Call(typeof(string), "IsNullOrEmpty", null,
                            Expression.Property(parameter, item.Name)));
...
/*                    case Query.Operators.ArrayContains:
                    {
                            /*var contains = Expression.Call(Expression.Property(parameter, item.Name), "Contains", null,
                                Expression.Constant(Convert.ChangeType(item.Value, realType), property.PropertyType));#1#

                            //                        var contains = Expression.Call(realType.GetMethod("Contains"),new[]{realType}) 
//                            new[] {1,2,3}.Contains()
                        MethodInfo method = typeof(Enumerable).GetMethod("Contains", BindingFlags.Static | BindingFlags.Public | BindingFlags.NonPublic);
                        ConstantExpression constant = Expression.Constant(item.Value, typeof(int[]));
                        MemberExpression member = Expression.PropertyOrField(parameter,item.Name);
                        expression = Append(expression, Expression.Lambda(Expression.Call(member, method, constant),parameter));
                        break;
                    }*/
                    case Query.Operators.Range:
                    {
                        ...
                        if (item.ValueMin != null)
                        {
                            ...
                            minExp = Expression.GreaterThanOrEqual(Expression.Property(parameter, item.Name),
                                Expression.Constant(minValue, property.PropertyType));
                        }

                        if (item.ValueMax != null)
                        {
                            var maxValue = Convert.ChangeType(item.ValueMax, realType);
                            maxExp = Expression.LessThanOrEqual(Expression.Property(parameter, item.Name),
                                Expression.Constant(maxValue, property.PropertyType));
                        }
